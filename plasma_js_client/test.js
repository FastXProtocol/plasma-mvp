...
const testTx = async () => {
    ...
    await logBalance();
    // console.log((await fastx.getUTXO()).data.result);

    let res = await fastx.sendEth(receiverAddress, 150);
...
const testPsTx = async () => {
    ...
    console.log("1. ps tranctions", (await fastx.getAllPsTransactions()).data.result);

    await fastx.sellToken("0x0", 0, 1);

    await logBalance();

    let psTransaction = (await fastx.getAllPsTransactions()).data.result[0];
    console.log(psTransaction);
    if (psTransaction) {
        await fastx.sendPsTransactionFill(psTransaction, 0, 0, 0, receiverAddress);
        console.log("2. ps tranctions", (await fastx.getAllPsTransactions()).data.result);
